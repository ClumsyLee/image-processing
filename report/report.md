...
![Zig-Zag](https://upload.wikimedia.org/wikipedia/commons/4/43/JPEG_ZigZag.svg)

我能想到的方法有：

* 基于状态机：以当前的移动方向为状态，具体状态转移关系如下
    - 向右且在上边缘 -> 向左下
    - 向右且在下边缘 -> 向右上
    - 向下且在左边缘 -> 向右上
    - 向下且在右边缘 -> 向左下
    ...
    - 向右上且在右边缘 -> 向下
    - 其他情况 -> 保持方向不变
* 查表：由于 JPEG 中只涉及到固定大小块（8x8）的 Zig-Zag，我们实际上可以提前建立好索引表，里面包含第几个元素是哪个的信息；
* 奇技淫巧：我从 `stackoverflow` 上学习到了一种方式：
...
理解了这段代码之后，我感觉三观受到了颠覆 orz

这段代码首先创建了一个矩阵，其中斜对角线方向上的元素大小相同，且每个元素比其左上方元素大1。

然后，它在刚刚的矩阵中加上一个大小在 `-0.5 - 0.5` 之间的分量，使得元素大小巧妙地变成了 Zig-Zag 序…

然后它愉快地取出了排序后每个元素的索引，返回 Zig-Zag 序列 orz。

我的内心几乎是崩溃的…构造的太巧妙了哇混蛋！

由于我们的任务很单一，我认为这里查表法是最好的选择。上面最后一种方法虽然精妙，但在本质上相当于每次都计算了一遍 8x8 Zig-Zag 序列，这明显是冗余的。

具体代码实现十分简单粗暴，但效率应该极高：

```matlab
function index = zigzag(r)
    if r ~= 8
        error(['No implement for zigzag(' int2str(r) ')'])
    end
    index = [1
             9
             2
...
```matlab
detect_face(img, v4, 55, 10, 0.35)
```

![L = 4 识别结果](detect_v4.png)

```matlab
detect_face(img, v5, 55, 10, 0.5)
```
...
```matlab
detect_face(imrotate(img, -90), v3, 55, 10, 0.23)
```

![顺时针旋转90度后识别结果](rotate.png)

#### 宽度拉伸为两倍

```matlab
detect_face(imresize(img2, [size(img2, 1), 2 * size(img2, 2)]), ...
            v3, 75, 10, 0.23)
```

![宽度拉伸为两倍后识别结果](resize.png)

#### 适当改变颜色

我们使用 `imadjust` 函数来调整图片的颜色，让图片看起来更有大片的感觉一些~

同时，由于颜色失真，我们将阈值适当调大为 0.4：

```matlab
detect_face(imadjust(img, [.1 .1 .1; 1 .85 .85], []), v3, 55, 10, 0.4)
```

识别结果如下：

![适当改变颜色后识别结果](imadjust.png)

可以看出，在上述几种情况下，我们的识别算法都很稳定：那位脸黑的队员仍然没能被识别出来，而其他人的脸仍然能被正确地识别出来。这说明，基于颜色的人脸识别确实具有抗旋转，拉伸和适当颜色改变的优点。

由于旋转前后，我们的算法中采样块的选取实际上是相同的，故抗旋转这一点特性我们可以轻松推断出。对于拉伸，由于其并未改变脸部的颜色特性，故我们的算法仍然能准确地识别出人脸。最后，对于适当改变颜色之后的图片，由于在颜色域上仍然只有人脸和模型最接近，故放宽阈值后仍然能正确识别。

### 4.4 如果可以重新选择人脸样本训练，你会怎么做？

我认为，首先训练样本应该尽可能大，而且能大致反映出现实中的人群比例，减小取样对模型带来的影响。具体可以包括以下几个特征：

- 年龄
- 性别
- 脸型（例如鼻子大小等，可能影响脸上的颜色分布）

其次，样本应该尽量排除背景与头发对识别结果的影响。这些区域一般与脸部的颜色相差较大，故其占的比例越大，所得到的训练结果就越不准确。

最后，如果可能，最好针对差异过大的人群分别训练。例如，对于白种人、黄种人和黑种人，如果在一同训练，最后得出的模型在颜色空间中可能会有 3 个峰，而每个峰的高度都较小，从而对哪种人的匹配效果都不好。

## 实验总结

这次实验的工作量可以用**极大**来形容。为了完成这个实验，我曾经有一段时间在寝室里丧心病狂，每天8点起床，写一上午代码之后吃外卖，然后写一下午代码之后吃外卖，最后写一晚上代码…到最后感觉整个人已经变得萌萌哒，一站起来就觉得头昏脑涨…

然后自然而然地病倒了orz

高烧躺在床上，满脑子是 JPEG 却昏得无法思考我终于明白了那句话：身体是最重要的…

不管怎么样，我觉得这次实验（用粗暴的手段）让我练习使用了 MATALB 的图像处理功能。不得不说这个功能确实十分强大！同时，我也试着使用了那种“没有接收返回值就把结果打印出来”的设计，确实能提高工作效率~
